...
namespace HeuristicLab.Problems.BinPacking3D.Sorting {
  internal class PackingItemClusteredAreaHeightSorter : IPackingItemClusteredSorter {
    public Permutation SortPackingItems(IList<PackingItem> items, PackingShape bin, double delta) {
      return items.SortByClusteredAreaHeight(bin, delta);
    }
    ...
    public Permutation SortPackingItemsByMaterial(IList<PackingItem> items, PackingShape bin, double delta) {
      return items.SortByMaterialClusteredAreaHeight(bin, delta);
    }
