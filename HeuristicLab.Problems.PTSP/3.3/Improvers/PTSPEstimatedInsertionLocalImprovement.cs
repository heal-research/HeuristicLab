...
namespace HeuristicLab.Problems.PTSP {
  ...
  public sealed class PTSPEstimatedInsertionLocalImprovement : SingleSuccessorOperator, IEstimatedPTSPOperator, IPermutationLocalImprovementOperator {
    ...
    public static void Improve(Permutation assignment, IProbabilisticTSPData data, DoubleValue quality, IntValue localIterations, IntValue evaluatedSolutions, bool maximization, int maxIterations, IEnumerable<BoolArray> realizations, CancellationToken cancellation) {
      for (var i = localIterations.Value; i < maxIterations; i++) {
        ...
        foreach (var move in ExhaustiveInsertionMoveGenerator.Generate(assignment)) {
          double moveQuality = PTSPEstimatedInsertionMoveEvaluator.EvaluateMove(assignment, move, data, realizations);
