...
namespace HeuristicLab.Problems.Dynamic {
  [StorableType("E3A4AB58-BAD6-40AD-A40C-320D1CFD8869")]
  public abstract class MultiObjectiveStatefulDynamicProblem<TEncoding, TSolution, TState> : MultiObjectiveDynamicBasicProblem<TEncoding, TSolution, TState>
    ...
    where TState : class, IDynamicProblemState<TState> 
  {
    ...
    #region Propeties
    public override bool[] Maximization => new[] { true, true };
    ...
    protected MultiObjectiveStatefulDynamicProblem() {
    ...
    [StorableConstructor]
    protected MultiObjectiveStatefulDynamicProblem(StorableConstructorFlag _) : base(_) { }

    protected MultiObjectiveStatefulDynamicProblem(MultiObjectiveStatefulDynamicProblem<TEncoding, TSolution, TState> original, Cloner cloner) : base(original, cloner) { }
    #endregion
    ...
    protected override void AnalyzeProblem(ResultCollection results, IRandom random, bool dummy) {
      ...
      ScatterPlot plot;
      IResult plotResult;
      if (!results.TryGetValue(StatePlotResultName, out plotResult))
        results.AddOrUpdateResult(StatePlotResultName, plot = CreateScatterPlot());
      else
        plot = (ScatterPlot)plotResult.Value;
      plot.Rows[StatePlotRowName].Points.Add(new Point2D<double>(plot.Rows[StatePlotRowName].Points.Count, StateVersion));
    }
