...
namespace HeuristicLab.Problems.DataAnalysis.Symbolic.Regression {
  [Item("SymbolicRegressionSingleObjectivePruningAnalyzer", "An analyzer that prunes introns from the population.")]
  [StorableType("F1180389-1393-4102-9EEC-E4F183A017F2")]
  public sealed class SymbolicRegressionSingleObjectivePruningAnalyzer : SymbolicDataAnalysisSingleObjectivePruningAnalyzer {
    private const string PruningOperatorParameterName = "PruningOperator";
    public IValueParameter<SymbolicRegressionSingleObjectivePruningOperator> PruningOperatorParameter {
      get { return (IValueParameter<SymbolicRegressionSingleObjectivePruningOperator>)Parameters[PruningOperatorParameterName]; }
    }
    ...
    private SymbolicRegressionSingleObjectivePruningAnalyzer(SymbolicRegressionSingleObjectivePruningAnalyzer original, Cloner cloner) : base(original, cloner) { }
    public override IDeepCloneable Clone(Cloner cloner) { return new SymbolicRegressionSingleObjectivePruningAnalyzer(this, cloner); }

    [StorableConstructor]
    private SymbolicRegressionSingleObjectivePruningAnalyzer(StorableConstructorFlag _) : base(_) { }

    public SymbolicRegressionSingleObjectivePruningAnalyzer() {
      Parameters.Add(new ValueParameter<SymbolicRegressionSingleObjectivePruningOperator>(PruningOperatorParameterName, "The operator used to prune trees", new SymbolicRegressionSingleObjectivePruningOperator(new SymbolicRegressionSolutionImpactValuesCalculator())));
    }
