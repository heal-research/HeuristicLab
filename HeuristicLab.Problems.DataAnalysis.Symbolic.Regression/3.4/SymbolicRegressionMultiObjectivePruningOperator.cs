...
namespace HeuristicLab.Problems.DataAnalysis.Symbolic.Regression {
  [StorableType("01066CA2-AF86-4B06-883B-656693465C1F")]
  [Item("SymbolicRegressionMultiObjectivePruningOperator", "An operator which prunes symbolic regression trees.")]
  public class SymbolicRegressionMultiObjectivePruningOperator : SymbolicDataAnalysisMultiObjectiveExpressionPruningOperator {
    private const string EvaluatorParameterName = "Evaluator";

    #region parameter properties
    public ILookupParameter<ISymbolicRegressionMultiObjectiveEvaluator> EvaluatorParameter => (ILookupParameter<ISymbolicRegressionMultiObjectiveEvaluator>)Parameters[EvaluatorParameterName];
    #endregion

    protected SymbolicRegressionMultiObjectivePruningOperator(SymbolicRegressionMultiObjectivePruningOperator original, Cloner cloner)
      : base(original, cloner) {
    }
    public override IDeepCloneable Clone(Cloner cloner) {
      return new SymbolicRegressionMultiObjectivePruningOperator(this, cloner);
    }

    [StorableConstructor]
    protected SymbolicRegressionMultiObjectivePruningOperator(StorableConstructorFlag _) : base(_) { }

    public SymbolicRegressionMultiObjectivePruningOperator(ISymbolicDataAnalysisSolutionImpactValuesCalculator impactValuesCalculator)
      : base(impactValuesCalculator) {
      Parameters.Add(new LookupParameter<ISymbolicRegressionMultiObjectiveEvaluator>(EvaluatorParameterName));
    }
    ...
    //  // skip the nodes corresponding to the ProgramRootSymbol and the StartSymbol

    //  var qualityForImpactsCalculation = double.NaN; // pass a NaN value initially so the impact calculator will calculate the quality
    ...
    //      out var impactValue, out var replacementValue, out var newQualityForImpactsCalculation, qualityForImpactsCalculation);
