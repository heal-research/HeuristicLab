...
namespace HeuristicLab.Optimization {
  ...
  public sealed class TimeLimitRun : NamedItem, IOptimizer, IStorableContent, INotifyPropertyChanged {
    ...
    private void MakeSnapshot() {
      ...
      Run run;
      if (StoreAlgorithmInEachSnapshot) {
        var temporarilySetStoreFlag = !Algorithm.StoreAlgorithmInEachRun;
        if (temporarilySetStoreFlag) Algorithm.StoreAlgorithmInEachRun = true;
        run = new Run(runName, Algorithm);
        if (temporarilySetStoreFlag) Algorithm.StoreAlgorithmInEachRun = false;
      } else {
        // duplicate code of Run.Initialize - necessary, because Algorithm property doesn't have a setter
        run = new Run() { Name = runName };
        var parameters = new Dictionary<string, IItem>();
        var results = new Dictionary<string, IItem>();
        algorithm.CollectParameterValues(parameters);
        algorithm.CollectResultValues(results);
        var cloner = new Cloner();
        foreach (var kvp in parameters.Select(x => new KeyValuePair<string, IItem>(x.Key, cloner.Clone(x.Value))).ToDictionary(x => x.Key, x => x.Value))
          run.Parameters.Add(kvp);
        foreach (var kvp in results.Select(x => new KeyValuePair<string, IItem>(x.Key, cloner.Clone(x.Value))).ToDictionary(x => x.Key, x => x.Value))
          run.Results.Add(kvp);
      }
      snapshots.Add(run);
    }
