...
namespace HeuristicLab.RealVector {
  public class SelfAdaptiveDiscreteRecombination : DiscreteRecombination {
    public override string Description {
      get {
        return @"Self adaptive Discrete/dominant recombination creates a new offspring by combining the alleles in the parents such that each allele is randomly selected from one parent. It will also use the same strategy to combine the endogenous strategy parameter vector.";
      }
    }

    public SelfAdaptiveDiscreteRecombination()
      : base() {
    ...
    public override IOperation Apply(IScope scope) {
      ...
      for (int i = 0; i < lambda; i++) {
        ...
        for (int x = 0; x < childGene.Length; x++) {
          int nextParent = random.Next(rho);
          childGene[x] = parents[nextParent][x];
          strategyParams[x] = parentsStrategy[nextParent][x];
        }
