  ...
  public class RunnerService : IRunnerService, IDisposable {
    ...
    public ExperimentKit PrepareExperiment(Algorithm algorithm, Problem problem) {
      ...
      try {
        ...
        dlo.LoadWith<AlgorithmParameter>(p => p.DataType);
        dlo.LoadWith<ProblemParameter>(p => p.DataType);
    ...
    private IQueryable<Experiment> CreateExperimentQuery(Algorithm algorithm, Problem problem, OKBDataContext okb) {
      var experimentQuery =
