...
namespace HeuristicLab.Hive.Client.Console
{
  ...
  public partial class HiveClientConsole : Form
  {
    ...
    private void ConnectToClient()
    {
      ...
      {
        ManageFatalException("Connection Error, check if Hive Client is running!", "Connection Error", ex);
      }
    }
    ...
    private void clientCommunicator_GetCurrentConnectionCompleted(object sender, GetCurrentConnectionCompletedEventArgs e)
    {
      ...
      {
        ManageFatalException("Connection Error, check if Hive Client is running!", "Connection Error", null);
      }
    }
    ...
    private void ManageFatalException(string body, string caption, Exception e)
    {
      if (!isfired)
      {
        isfired = true;
        refreshTimer.Stop();
        DialogResult res = MessageBox.Show(body, caption, MessageBoxButtons.OK, MessageBoxIcon.Error);
