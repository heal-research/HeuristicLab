...
namespace HeuristicLab.Problems.Knapsack {
  ...
  public class KnapsackEvaluator : SingleSuccessorOperator, IKnapsackEvaluator {
    ...
    public sealed override IOperation Apply() {
      ...
      int capacity = KnapsackCapacityParameter.ActualValue.Value;
      double penalty = PenaltyParameter.ActualValue.Value;

      if (WeightsParameter.ActualValue.Length != ValuesParameter.ActualValue.Length)
      ...
      for (int i = 0; i < v.Length; i++) {
        if (v[i]) {
          weight += WeightsParameter.ActualValue[i];
          value += ValuesParameter.ActualValue[i];
        }
      }

      if (weight > capacity) {
        quality =
          value - penalty * (weight - capacity);
      } else {
        quality = value;
      }
