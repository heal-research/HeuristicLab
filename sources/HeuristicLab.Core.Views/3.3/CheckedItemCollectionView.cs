...
namespace HeuristicLab.Core.Views {
  ...
  public partial class CheckedItemCollectionView<T> : ItemView where T : class, IItem {
    ...
    public CheckedItemCollectionView(ICheckedItemCollection<T> content)
      : this() {
      Content = content;
    }
    ...
    protected virtual void Content_ItemsChecked(object sender, CollectionItemsChangedEventArgs<T> e) {
      if (InvokeRequired)
        Invoke(new CollectionItemsChangedEventHandler<T>(Content_ItemsChecked), sender, e);
      else {
        foreach (T item in e.Items) {
          foreach (ListViewItem listViewItem in GetListViewItemsForItem(item)) {
