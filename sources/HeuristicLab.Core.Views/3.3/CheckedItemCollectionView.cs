...
namespace HeuristicLab.Core.Views {
  ...
  public partial class CheckedItemCollectionView<T> : ItemCollectionView<T> where T : class, IItem {
    ...
    protected override void itemsListView_DragEnter(object sender, DragEventArgs e) {
      ...
      var data = e.Data.GetData(HeuristicLab.Common.Constants.DragDropDataFormat) as ICheckedItemCollection<T>;
      if (data != null)
    ...
    protected override void itemsListView_DragDrop(object sender, DragEventArgs e) {
      ...
      if (data != null) {
        for (int i = 0; i < Content.Count; i++) {
          Content.SetItemCheckedState(Content.ElementAt(i), data.ItemChecked(data.ElementAt(i)));
        }
      } else
