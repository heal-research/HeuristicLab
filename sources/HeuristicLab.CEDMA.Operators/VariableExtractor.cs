...
namespace HeuristicLab.CEDMA.Operators {
  public class VariableExtractor : OperatorBase {
    ...
    public override IOperation Apply(IScope scope) {
      ...
      using(ChannelFactory<IDatabase> factory = new ChannelFactory<IDatabase>(binding)) {
        ...
        IList<ItemEntry> entries = database.GetItems(predicate.Guid, property.Guid);
        if(entries.Count == 1) {
          IItem item = (IItem)PersistenceManager.RestoreFromGZip(entries[0].RawData);
          scope.AddVariable(new Variable(variableName, item));
        } else {
          ItemList<IItem> itemList = new ItemList<IItem>();
          foreach(ItemEntry entry in entries) {
            IItem item = (IItem)PersistenceManager.RestoreFromGZip(entry.RawData);
            itemList.Add(item);
          }
          scope.AddVariable(new Variable(variableName, itemList));
        }
        return null;
      }
    }
