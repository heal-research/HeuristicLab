...
namespace HeuristicLab.GP.StructureIdentification.ConditionalEvaluation {
  public class ConditionalMeanAbsolutePercentageOfRangeErrorEvaluator : ConditionalEvaluatorBase {
...
the mean of the absolute percentage error (scale invariant) relative to the range of the target varibale of estimated values vs. real values of 'TargetVariable'.";
    ...
    public override double Evaluate(double[,] values) {
      try {
        return SimpleMeanAbsolutePercentageOfRangeErrorEvaluator.Calculate(values);
      }
