...
FOR /F "skip=1 tokens=1-2 delims=:" %%G IN (%ProjectDir%\Files.txt) DO copy "%SolutionDir%\%%G\%Outdir%\%%H" "%target%"


echo "Platform: %Platform%, architecture: %PROCESSOR_ARCHITECTURE%"
if "%Platform%" == "x86" (   
  FOR /F "skip=1 tokens=*" %%G IN (%ProjectDir%\Files.x86.txt) DO copy "%SolutionDir%\%%G" "%target%"
) else if "%Platform%" == "x64" ( 
  FOR /F "skip=1 tokens=*" %%G IN (%ProjectDir%\Files.x64.txt) DO copy "%SolutionDir%\%%G" "%target%"
