...
namespace HeuristicLab.Problems.Instances.DataAnalysis {
  public class RippleFunction : ArtificialRegressionDataDescriptor {
    ...
    protected override int TestPartitionEnd { get { return 1300; } }

    protected override List<List<double>> GenerateValues() {
      List<List<double>> data = new List<List<double>>();
      for (int i = 0; i < AllowedInputVariables.Count(); i++) {
        data.Add(ValueGenerator.GenerateUniformDistributedValues(TrainingPartitionEnd, 0.05, 6.05).ToList());
      }

      for (int i = 0; i < AllowedInputVariables.Count(); i++) {
        data[i].AddRange(ValueGenerator.GenerateUniformDistributedValues(TrainingPartitionEnd, -0.25, 6.35));
      }
