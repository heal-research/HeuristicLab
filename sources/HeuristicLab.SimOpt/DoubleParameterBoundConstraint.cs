...
namespace HeuristicLab.SimOpt {
  public class DoubleParameterBoundConstraint : ConstraintBase {
    ...
    public DoubleParameterBoundConstraint()
      : this(double.MinValue, double.MaxValue, "undefined") {
    }

    public DoubleParameterBoundConstraint(double lowerBound, double upperBound, string parameterName)
      : this(lowerBound, true, upperBound, true, parameterName) {
    }

    public DoubleParameterBoundConstraint(double lowerBound, bool lowerBoundIncluded, double upperBound, bool upperBoundIncluded, string parameterName)
      : base() {
    ...
    public override bool Check(IItem data) {
      ...
      for (int i = 0; i < list.Count; i++) {
        if (list[i] is IVariable && ((IVariable)list[i]).Name.Equals(parameterName) && ((IVariable)list[i]).Value is DoubleData) {
    ...
    public override IView CreateView() {
      return new DoubleParameterBoundConstraintView(this);
    }

    public override object Clone(IDictionary<Guid, object> clonedObjects) {
      DoubleParameterBoundConstraint clone = new DoubleParameterBoundConstraint();
