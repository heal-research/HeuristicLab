...
namespace HeuristicLab.Problems.DataAnalysis.Views {
  ...
  public partial class RegressionEnsembleSolutionScatterPlotView : ItemView, IRegressionEnsembleSolutionEvaluationView {
    ...
    public RegressionEnsembleSolutionScatterPlotView()
      : base() {
      InitializeComponent();
    ...
    private void UpdateSeries() {
      if (InvokeRequired) Invoke((Action)UpdateSeries);
        ...
        var trainingIndizes = Enumerable.Range(Content.ProblemData.TrainingPartition.Start, Content.ProblemData.TrainingPartition.End - Content.ProblemData.TrainingPartition.Start);
        var testIndizes = Enumerable.Range(Content.ProblemData.TestPartition.Start, Content.ProblemData.TestPartition.End - Content.ProblemData.TestPartition.Start);
        ...
        if (this.chart.Series[TRAINING_SERIES].Points.Count > 0)
          this.chart.Series[TRAINING_SERIES].Points.DataBindXY(Content.EstimatedTrainingValues.ToArray(), "",
            dataset.GetEnumeratedVariableValues(targetVariableName, trainingIndizes).ToArray(), "");
        if (this.chart.Series[TEST_SERIES].Points.Count > 0)
          this.chart.Series[TEST_SERIES].Points.DataBindXY(Content.EstimatedTestValues.ToArray(), "",
           dataset.GetEnumeratedVariableValues(targetVariableName, testIndizes).ToArray(), "");
      ...
      if (series.Points.Count > 0) {  //checks if series is shown
        ...
        switch (series.Name) {
          ...
          case TRAINING_SERIES:
            ...
            targetValues = Content.ProblemData.Dataset.GetEnumeratedVariableValues(targetVariableName, trainingIndizes).ToArray();
            break;
          case TEST_SERIES:
            predictedValues = Content.EstimatedTestValues.ToArray();
            targetValues = Content.ProblemData.Dataset.GetEnumeratedVariableValues(targetVariableName, testIndizes).ToArray();
            break;
        }
