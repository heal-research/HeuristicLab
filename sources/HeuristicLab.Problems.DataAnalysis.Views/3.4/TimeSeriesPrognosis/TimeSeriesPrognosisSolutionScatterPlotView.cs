...
namespace HeuristicLab.Problems.DataAnalysis.Views {
  ...
  public partial class TimeSeriesPrognosisSolutionScatterPlotView : DataAnalysisSolutionEvaluationView {
    ...
    public TimeSeriesPrognosisSolutionScatterPlotView()
      : base() {
      InitializeComponent();
      ...
      this.chart.ChartAreas[0].AxisX.Title = "Prognosed Values";
    ...
    private void UpdateSeries() {
      if (InvokeRequired) Invoke((Action)UpdateSeries);
        ...
        if (this.chart.Series[ALL_SERIES].Points.Count > 0)
          this.chart.Series[ALL_SERIES].Points.DataBindXY(Content.PrognosedValues.ToArray(), "",
            dataset.GetDoubleValues(targetVariableName).ToArray(), "");
        if (this.chart.Series[TRAINING_SERIES].Points.Count > 0)
          this.chart.Series[TRAINING_SERIES].Points.DataBindXY(Content.PrognosedTrainingValues.ToArray(), "",
        ...
        if (this.chart.Series[TEST_SERIES].Points.Count > 0)
          this.chart.Series[TEST_SERIES].Points.DataBindXY(Content.PrognosedTestValues.ToArray(), "",
           dataset.GetDoubleValues(targetVariableName, Content.ProblemData.TestIndizes).ToArray(), "");

        double max = Content.PrognosedTrainingValues.Concat(Content.PrognosedTestValues.Concat(Content.PrognosedValues.Concat(dataset.GetDoubleValues(targetVariableName)))).Max();
        double min = Content.PrognosedTrainingValues.Concat(Content.PrognosedTestValues.Concat(Content.PrognosedValues.Concat(dataset.GetDoubleValues(targetVariableName)))).Min();
    ...
    private void ToggleSeriesData(Series series) {
      if (series.Points.Count > 0) {  //checks if series is shown
        ...
        switch (series.Name) {
          case ALL_SERIES:
            predictedValues = Content.PrognosedValues.ToArray();
