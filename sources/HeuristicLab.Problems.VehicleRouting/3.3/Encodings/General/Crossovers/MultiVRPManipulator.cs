...
namespace HeuristicLab.Problems.VehicleRouting.Encodings.General {
  [Item("MultiVRPManipulator", "Randomly selects and applies one of its manipulators every time it is called.")]
  [StorableClass]
  public class MultiVRPManipulator : StochasticMultiBranch<IVRPManipulator>, IVRPManipulator, IStochasticOperator {
    ...
    [StorableConstructor]
    private MultiVRPManipulator(bool deserializing) : base(deserializing) { }
    public MultiVRPManipulator()
      : base() {
      ...
      foreach (Type type in ApplicationManager.Manager.GetTypes(typeof(IVRPManipulator))) {
        if (!typeof(MultiOperator<IVRPManipulator>).IsAssignableFrom(type))
          Operators.Add((IVRPManipulator)Activator.CreateInstance(type), true);
      }
    }
