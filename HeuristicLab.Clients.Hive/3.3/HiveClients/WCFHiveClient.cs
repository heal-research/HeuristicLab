...
namespace HeuristicLab.Clients.Hive {
  [Item("WCFHiveClient", "Hive client.")]
  public sealed class WCFHiveClient : IHiveClientImplementation {
    private static WCFHiveClient instance;
    public static WCFHiveClient Instance {
      get {
        if (instance == null) instance = new WCFHiveClient();
        return instance;
      }
    }
    ...
    public void Store(IHiveItem item, CancellationToken cancellationToken) {
      if (item.Id == Guid.Empty) {
        if (item is RefreshableJob) {
          item.Id = WCFHiveClient.Instance.UploadJob((RefreshableJob)item, cancellationToken);
        }
