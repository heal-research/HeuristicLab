...
namespace HeuristicLab.Build {
  public class MergeConfigs : Task {
    ...
    public override bool Execute() {
      ...
      Log.LogMessage("ConfigMerger: Merge source:      \"" + SolutionDir + "\"");
      Log.LogMessage("ConfigMerger: Merge destination: \"" + TargetDir + "\"");

      string targetConfigPath = $"{TargetDir}HeuristicLab 3.3.exe.config";
      File.Copy($"{SolutionDir}\\HeuristicLab\\3.3\\app.config", targetConfigPath, true);

      var configs = Directory.EnumerateFiles(TargetDir, "*.dll.config");
      foreach (var sourceConfigPath in configs) {
        XmlDocument source = new XmlDocument();
        source.Load(sourceConfigPath);
        XmlDocument destination = new XmlDocument();
        destination.Load(targetConfigPath);
        ...
        Merge(sourceNode, destinationNode, destination, "/configuration");
        destination.Save(targetConfigPath);
      }
    ...
    private void Merge(XmlNode source, XmlNode destination, XmlDocument document, string root) {
      try {
        if (source != null) {
          if (destination == null) {
    ...
    private string BuildXPathString(XmlNode node) {
