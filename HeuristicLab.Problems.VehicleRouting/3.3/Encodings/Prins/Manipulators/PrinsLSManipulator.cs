...
namespace HeuristicLab.Problems.VehicleRouting.Encodings.Prins {
  ...
  [StorableClass]
  public abstract class PrinsLSManipulator : PrinsManipulator {
    ...
    protected PrinsLSManipulator(PrinsLSManipulator original, Cloner cloner) : base(original, cloner) { }
    ...
    protected double GetQuality(PrinsEncoding individual) {
      return VRPEvaluator.Evaluate(individual,
        ...
        UseDistanceMatrixParameter.ActualValue).Quality;
    }
    ...
    private Tour FindTour(PrinsEncoding individual, int city) {
      ...
      List<Tour> tours = individual.GetTours(DistanceMatrixParameter);
      int i = 0;

      while (found == null && i < tours.Count) {
        if (tours[i].Cities.Contains(city))
          found = tours[i];
    ...
    private void M2(PrinsEncoding parent, PrinsEncoding child, int u, int v) {
      if (u != depot) {
        Tour tour = FindTour(child, u + 1);
        int iu = tour.Cities.IndexOf(u + 1);
        if (iu < tour.Cities.Count - 1) {
          int x = tour.Cities[iu + 1] - 1;
    ...
    private void M6(PrinsEncoding parent, PrinsEncoding child, int u, int v) {
      if (u != depot && v != depot) {
        ...
        if (iu < tour.Cities.Count - 1) {
          ...
          int iv = tour.Cities.IndexOf(v + 1);
          if (iv < tour.Cities.Count - 1) {
            int y = tour.Cities[iv + 1] - 1;
    ...
    private void M7(PrinsEncoding parent, PrinsEncoding child, int u, int v) {
      if (u != depot && v != depot) {
        ...
        if (tu.Cities[0] == tv.Cities[0]) {
          int iu = tu.Cities.IndexOf(u + 1);
          if (iu < tu.Cities.Count - 1) {
            int x = tu.Cities[iu + 1] - 1;

            int iv = tv.Cities.IndexOf(v + 1);
            if (iv < tv.Cities.Count - 1) {
              int y = tv.Cities[iv + 1] - 1;
