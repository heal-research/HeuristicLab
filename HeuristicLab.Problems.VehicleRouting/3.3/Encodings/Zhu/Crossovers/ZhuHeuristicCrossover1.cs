  ...
  public sealed class ZhuHeuristicCrossover1 : ZhuCrossover {
    ...
    protected override ZhuEncoding Crossover(IRandom random, ZhuEncoding parent1, ZhuEncoding parent2) {
      ...
      while (i != predecessor) {
        ...
        if (VRPUtilities.GetDistance(
          child[i] + 1, parent1[(i + 1) % child.Length] + 1,
          CoordinatesParameter.ActualValue, DistanceMatrixParameter,
          UseDistanceMatrixParameter.ActualValue)
          <
          VRPUtilities.GetDistance(
          child[i] + 1, parent2[(i + 1) % child.Length] + 1,
          CoordinatesParameter.ActualValue, DistanceMatrixParameter,
          UseDistanceMatrixParameter.ActualValue)) {
