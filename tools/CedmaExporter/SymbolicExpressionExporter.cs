...
namespace CedmaExporter {
  public class SymbolicExpressionExporter : IFunctionTreeExporter, IFunctionTreeNameGenerator {
    ...
    public SymbolicExpressionExporter(Dataset ds) {
      this.dataset = ds;
    }
    ...
    private string ExportFunction(IFunction function, IFunctionTree tree) {
      ...
      if (function is Differential) return ((Differential)function).ExportToScheme(tree, dataset);
      ...
      if (function is Variable) return ((Variable)function).ExportToScheme(tree, dataset);
  ...
  internal static class SchemeExporterExtensions {
    ...
    public static string ExportToScheme(this Variable variable, IFunctionTree tree, Dataset ds) {
      return "(variable " + tree.GetLocalVariable(Variable.WEIGHT).Value + " " +
        ds.GetVariableName(int.Parse(tree.GetLocalVariable(Variable.INDEX).Value.ToString())) + " " + tree.GetLocalVariable(Variable.OFFSET).Value + ")";
    }
    public static string ExportToScheme(this Differential differential, IFunctionTree tree, Dataset ds) {
      return "(differential " + tree.GetLocalVariable(Differential.WEIGHT).Value + " " +
        ds.GetVariableName(int.Parse(tree.GetLocalVariable(Differential.INDEX).Value.ToString())) + " " + tree.GetLocalVariable(Differential.OFFSET).Value + ")";
    }
