...
namespace HeuristicLab.Problems.Instances.DataAnalysis {
  public class Feynman3 : FeynmanDescriptor {
    private readonly int testSamples;
    private readonly int trainingSamples;

    public Feynman3() : this((int) DateTime.Now.Ticks, 10000, 10000, null) { }

    public Feynman3(int seed) {
      Seed            = seed;
    ...
    public Feynman3(int seed, int trainingSamples, int testSamples, double? noiseRatio) {
      Seed                 = seed;
    ...
    public override string Name {
      get {
        return string.Format(
          "I.6.20b exp(-((theta-theta1)/sigma)**2/2)/(sqrt(2*pi)*sigma) | {0} samples | {1}",
    ...
    protected override string[] AllowedInputVariables { get { return new[] {"sigma", "theta", "theta1"}; } }
    ...
    protected override List<List<double>> GenerateValues() {
      ...
      var theta1 = ValueGenerator.GenerateUniformDistributedValues(rand.Next(), TestPartitionEnd, 1, 3).ToList();
      ...
      for (var i = 0; i < sigma.Count; i++) {
        var res = Math.Exp(-Math.Pow((theta[i] - theta1[i]) / sigma[i], 2) / 2) / (Math.Sqrt(2 * Math.PI) * sigma[i]);
        f.Add(res);
      }
