...
namespace HeuristicLab.Problems.Instances.DataAnalysis {
  public class Feynman22 : FeynmanDescriptor {
    ...
    public override string Name {
      get {
        return string.Format("I.18.12 r*F*sin(theta) | {0} samples | {1}", trainingSamples,
    ...
    protected override List<List<double>> GenerateValues() {
      ...
      if (noiseRatio != null) {
        ...
        var sigma_noise = (double) noiseRatio * tau.StandardDeviationPop();
        tau_noise.AddRange(tau.Select(md => md + NormalDistributedRandom.NextDouble(rand, 0, sigma_noise)));
        data.Remove(tau);
        data.Add(tau_noise);
      }
