...
namespace HeuristicLab.Problems.Instances.DataAnalysis {
  public class Feynman49 : FeynmanDescriptor {
    ...
    public override string Name {
      get {
        return string.Format("I.47.23 sqrt(gamma*pr/rho) | {0} samples | {1}", trainingSamples,
    ...
    protected override List<List<double>> GenerateValues() {
      ...
      if (noiseRatio != null) {
        ...
        var sigma_noise = (double) noiseRatio * c.StandardDeviationPop();
        c_noise.AddRange(c.Select(md => md + NormalDistributedRandom.NextDouble(rand, 0, sigma_noise)));
        data.Remove(c);
        data.Add(c_noise);
      }
