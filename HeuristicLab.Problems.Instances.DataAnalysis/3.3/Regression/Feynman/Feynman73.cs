...
namespace HeuristicLab.Problems.Instances.DataAnalysis {
  public class Feynman73 : FeynmanDescriptor {
    ...
    public override string Name {
      get {
        return string.Format("II.27.16 epsilon*c*Ef**2 | {0} samples | {1}", trainingSamples,
    ...
    protected override List<List<double>> GenerateValues() {
      ...
      if (noiseRatio != null) {
        ...
        var sigma_noise = (double) noiseRatio * flux.StandardDeviationPop();
        flux_noise.AddRange(flux.Select(md => md + NormalDistributedRandom.NextDouble(rand, 0, sigma_noise)));
        data.Remove(flux);
        data.Add(flux_noise);
      }
